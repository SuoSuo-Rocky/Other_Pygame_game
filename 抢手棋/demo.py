...
class MySprite(pygame.sprite.Sprite):
    ...
    def charge_succ(self, stop_index):
        """ 到达终点判断 """
        if self.current_index == stop_index:
            self.auto_move_switch = False
            self.is_move = False
            self.step_num = 0
...
def  charge_game_over():
    """ 游戏结束， 其中一人到达终点 """
    global is_sample, Dir_li, game_switch, random_switch
    print(f'super_man_cur-index = {super_man.current_index}---- shadow_index = {shadow_man.current_index}')
    if super_man.current_index ==  (len(Dir_li) - 1 ) or \
            shadow_man.current_index ==  (len(Dir_li) - 1 ):
        game_switch = False
        random_switch = False
        super_man.charge_succ(len(Dir_li) - 1)
        shadow_man.charge_succ(len(Dir_li) - 1)
        is_sample = False
        return False
    else:
        return True
...
button = Button(screen, 195, 388, 150, 64,  text = "Click", \
                font=pygame.font.Font("songti.otf", 40,), fontSize = 30, \
...
while running:
    ...
    # print(f'game_switch = {game_switch}')
    ...
    # if not game_switch:
    #     print(f'is_move = {super_man.is_move}-- auto_move = {super_man.auto_move_switch}')
    #     print(f'is_move = {shadow_man.is_move}-- auto_move = {shadow_man.auto_move_switch}+++++++++++++++++')
