...
class Baffle(pygame.sprite.Sprite):
    ...
    def __init__(self, w, h, **kwargs):
        super(Baffle, self).__init__()
        self.image = pygame.Surface((w, h))
        self.rect = self.image.get_rect(**kwargs)

    def update(self, *args):
        if is_game_running:
            for k, obj in enumerate(baffle_group.sprites()):
                obj.move(k)
    ...
    def create_baffle_group():
        ...
        for i in range(BAFFLE_GROUP_NUM):
            ...
            down_obj = Baffle(BAFFLE_WIDTH, BAFFLE_HEIGHT, \
                              topleft = topleft)
            baffle_group.add(up_obj)
            baffle_group.add(down_obj)
    ...
    def move(self, index):
        ...
        if index in [0, 2, 4, 6]:
            ...
            if not self.rect.colliderect(screen.get_rect()) and \
                    self.rect.left < 0:
                print("过界 ", index)
                self.rect.left = baffle_group.sprites()[front].rect.right + \
                                 VERTICAL_BAFFLE_INTERVAL
                baffle_group.sprites()[index + 1].rect.left =  \
                    baffle_group.sprites()[front].rect.right + \
                    VERTICAL_BAFFLE_INTERVAL
...
def collision_test():
    ...
    """
    # 猫头鹰和挡板
    for obj in baffle_group.sprites():
...
def reset():
    ...
    游戏复位
    """
    global baffle_group, is_game_running
    # 清空挡板对象
    baffle_group.sprites().clear()
    # 重新创建挡板
    baffle_group = Baffle.create_baffle_group()
    # 重置猫头鹰初始位置
    eagle.rect.center = BIRD_INIT_POSI
    # 游戏开始
...
eagle, floor, baffle_group = init_sprite() # 初始化精灵
...
while True:
    ...
    # baffle_group.draw(screen)
    for obj in baffle_group.sprites():# 画挡板
        obj.draw(screen)
